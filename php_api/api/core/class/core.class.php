<?php
	...
	**/
	...
	class core {
		...
		public function app_registration($email, $fname, $mname, $lname, $address1, $address2, $city, $province, $zip, $dob, $gender, $mobileNum, $platform, $landlineNum, $password) {
			...
			if ($result->num_rows > 0) {
				...
				if ($row['result'] != 'Failed') {
					...
					array_push($output, array("description"=>"Please check your email. If you don't receive an email in 5 minutes, kindly check the spam folder."));
					$sql->close();
					...
					$message .= '<p style="font-family: Tahoma !important;">You are one step away to complete the registration process. Please activate your account by clicking the link provided below:</p>';
					$message .= '<p style="font-family: Tahoma !important;"><br/><a href="'.PATH.'activation.php?email='.$email.'&activation='.$activation.'" style="background:#000; color:#fff; text-decoration:none;">Activate</a></p>';
		...
		/********** Facebook Login **********/
		public function fb_login($email, $pid, $platform, $deviceID, $fname, $mname, $lname, $dob, $gender) {
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			$file_name = self::$tmp_file_name;
			$error000 = self::$tmp_error000;
			...
			if (!$sql->bind_param('sssssssss', $email, $pid, $deviceID, $platform, $fname, $mname, $lname, $dob, $gender)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: fb_login] [Param: $email, $pid, $platform, $deviceID, $fname, $mname, $lname, $dob, $gender]");
			    die($error000);
				return;
			}
		...
		public function update_member_profile($memberID, $fname, $lname, $gender, $address1, $mobileNum, $dob) {
			$mysqli = self::$tmp_mysqli;
			$logs = self::$tmp_logs;
			$file_name = self::$tmp_file_name;
			$error000 = self::$tmp_error000;
			...
			if (!$sql->bind_param('sssssss', $memberID, $fname, $lname, $gender, $address1, $mobileNum, $dob)) {
				$logs->write_logs('MySQLi Bind Param', $file_name, "Bind Param failed: (" . $mysqli->errno . ") " . $mysqli->error . "\t [Procedure: core_update_member_data] [Param: $memberID, $fname, $lname, $gender, $address1, $mobileNum, $dob]");
			    die($error000);
				return;
			}
			...
			if ($result->num_rows > 0) {
				...
				if ($row['result'] == 'Success') {
					$logs->write_logs('Update Member Profile', $file_name, "[Param: $memberID, $fname, $lname, $gender, $address1, $mobileNum, $dob]\t Status: [Success]");
					$sql->close();
					// return json_encode(array(array("response"=>"Success")));
					return $this->complete_data($memberID);
				} else {
					$logs->write_logs('Update Member Profile', $file_name, "[Message: Unable to complete process.] [Param: $memberID, $fname, $lname, $gender, $address1, $mobileNum, $dob]");
					$sql->close();
